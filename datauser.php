		...
		<div class="content">
			...
			<div class="table-responsive">
				<table class="table table-striped table-hover table-bordered ">
					...
					?>
					<?php
					$cek = mysqli_query($koneksi, "SELECT * FROM user WHERE level = 'User'");
					if (mysqli_num_rows($cek) == 0) {
						...
						while ($row = mysqli_fetch_assoc($cek)) {
							...
							<tr>
								...
								<td>' . $row['email'] . '</td>
								<td>
							<a href="datauser.php?aksi=delete&id=' . $row['id'] . '" title="Hapus Data" data-toggle="tooltip" onclick="return confirm(\'Anda yakin akan menghapus data ' . $row['username'] . '?\')" class="btn btn-danger btn-sm"><span class="glyphicon glyphicon-trash" aria-hidden="true"></span></a>
								</td>
							</tr>
							';
							$no++;
